...
namespace PTC.DOTIC.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ITransientDependency
    {
        ...
        {
            using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("PTC.DOTIC.Emailing.EmailTemplates.default.html"))
            {
